...
当前摘要使用固定版本 `gemini-3.6-flash`。针对每天20次的 Gemini 请求限制，默认最多总结18篇、关闭额外重试并关闭基于 LLM 的作者单位提取，把请求额度优先留给论文摘要；失败时使用原始摘要，邮件不会留下空白 TLDR。`llm.summary.max_input_tokens` 控制单篇论文最多提供给 Gemini 的输入长度，当前为40000。PubMed 作者单位仍直接来自 NCBI，不消耗 Gemini 请求。
